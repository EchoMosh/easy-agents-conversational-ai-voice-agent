...
export function AgentVoiceCall({
  ...
  useEffect(() => {
    if (open) {
      if (!vapiRef.current) {
        console.log("AgentVoiceCall: Dialog opened, initializing Vapi.");
        initializeVapi();
        // Agent update logic is now handled in TestAgentDialog
      } else {
        console.log("AgentVoiceCall: Dialog opened, Vapi instance already exists.");
      }
    }
    
    return () => {
      console.log("AgentVoiceCall: useEffect[open] cleanup. Dialog is closing or component unmounting.");
      resetState(); 
    };
  }, [open]); 
  ...
  const initializeVapi = () => {
    if (typeof navigator.mediaDevices === 'undefined' || typeof navigator.mediaDevices.enumerateDevices === 'undefined') {
      console.error("MediaDevices API (navigator.mediaDevices.enumerateDevices) is not available. This can happen in insecure contexts (non-HTTPS, excluding localhost) or unsupported browsers.");
      setErrorDetails("Required microphone access API is not available. Please ensure you are on a secure (HTTPS) connection or using localhost, and that your browser supports this feature.");
      setCallStatus("error");
      return;
    }
    ...
    try {
      console.log("Initializing Vapi with key:", vapiKey.substring(0, 5) + "...");
  ...
  const setupVapiEventListeners = () => {
    ...
    vapiRef.current.on("call-start", () => {
      console.log("Call started event received from Vapi.");
    ...
    vapiRef.current.on("message", (msg: any) => {
      console.log("Message received from Vapi:", JSON.stringify(msg, null, 2));
      ...
      if (msg.role === "user") {
        if (msg.type === "transcript") {
          ...
          if (isPartial) {
            setMessages(prev => {
              const lastUserMessageIndex = [...prev].reverse().findIndex(m => m.role === "user" && !m.isFinal);
              if (lastUserMessageIndex >= 0) { // Corrected: was lastUserMessageIdRef.current
                const actualIndex = prev.length - 1 - lastUserMessageIndex;
                const updated = [...prev];
                updated[actualIndex] = { ...updated[actualIndex], content };
                return updated;
              } else {
                return [...prev, { id: Date.now().toString(), role: "user", content, timestamp: new Date(), isFinal: false }];
              }
            });
          } else if (isFinal) {
            setMessages(prev => {
              ...
              if (lastUserMessageIndex >= 0) { // Corrected: was lastUserMessageIdRef.current
                const actualIndex = prev.length - 1 - lastUserMessageIndex;
                const updated = [...prev];
                updated[actualIndex] = { ...updated[actualIndex], content, isFinal: true };
                return updated;
              } else {
                return [...prev, { id: Date.now().toString(), role: "user", content, timestamp: new Date(), isFinal: true }];
              }
            });
          }
        } else {
          console.warn("Received user-role message that is not a transcript:", msg);
        }
        ...
        if (msg.type === "transcript" && typeof msg.transcript === 'string') {
          ...
          // Check if this is a final transcript - this is the key to proper sentence boundaries
          isFinalTranscript = msg.transcriptType === "final";
          console.log("Assistant transcript:", assistantContent, "isFinal:", isFinalTranscript);
        } else if (msg.type === "assistant" && typeof msg.text === 'string') {
          assistantContent = msg.text;
          // For non-transcript messages, we'll treat them as regular updates
          // but can optionally check for sentence boundaries later
        ...
        if (assistantContent) {
          if (currentAgentMessageIdRef.current) {
            ...
            if (isFinalTranscript) {
              ...
              if (isAgentSpeaking) {
                const newAgentMessageId = Date.now().toString() + "-agent-speech-" + Math.random().toString(36).substring(7);
                setMessages(prev => [...prev, {
                  id: newAgentMessageId,
                  role: "agent",
            ...
            console.log("Creating new bubble for assistant content with no current ref");
            const newAgentMessageId = Date.now().toString() + "-agent-text-" + Math.random().toString(36).substring(7);
            setMessages(prev => [...prev, {
              id: newAgentMessageId,
              role: "agent",
          ...
          if (msg.type === "function-call") {
            console.log("Function call from assistant (not displayed in chat):", msg.functionCall);
          } else if (msg.type === "tool-calls") {
            console.log("Tool calls from assistant (not displayed in chat):", msg.toolCalls);
          } else {
            console.warn("Received assistant-role message with no displayable content:", msg);
          }
        }
      } else {
        console.warn("Unhandled Vapi message with unknown role:", msg.role, "Full message:", msg);
      }
    });
  };
  ...
  const handleStartCall = async () => {
    ...
    try {
      ...
      const assistantId = agent.v_agent_id || agent.elevenlabs_agent_id || agent.id;
      
      console.log("Starting call with assistant ID:", assistantId);
      
      if (!assistantId) {
        throw new Error("No valid assistant ID found. Please make sure the agent has a v_agent_id field.");
      }
      
      // Check if the ID looks like a Vapi ID (typically a UUID)
      const looksLikeVapiId = /^[a-f0-9-]{36}$/.test(assistantId) || assistantId.startsWith('asst_');
      if (!looksLikeVapiId) {
        console.warn("The assistant ID doesn't appear to be a valid Vapi ID format:", assistantId);
      }
      ...
      if (error instanceof Error) {
        ...
        errorMessage = "Connection failed with undefined error. This often indicates a network issue or invalid assistant ID.";
      ...
      if (!assistantId) {
        setErrorDetails("No valid assistant ID found. Please make sure the agent has been properly created in Vapi.");
      } else if (!/^[a-f0-9-]{36}$/.test(assistantId) && !assistantId.startsWith('asst_')) {
        setErrorDetails(`The assistant ID "${assistantId}" doesn't appear to be a valid Vapi assistant ID. Please verify you've created an assistant in the Vapi dashboard.`);
      }
    }
  };
  ...
  const checkMicrophonePermissions = async () => {
    try {
      ...
      if (permissionStatus.state === 'denied') {
        setErrorDetails("Microphone access denied. Please allow microphone access in your browser settings.");
      }
    } catch (error) {
  ...
  const handleRetry = async () => {
    if (retryCount >= maxRetries) {
      setErrorDetails(`Failed after ${maxRetries} attempts. Please check your connection and try again later.`);
      return;
    }
    ...
    setTimeout(() => {
      ...
      if (vapiRef.current) {
        try {
          console.log("Stopping existing Vapi instance before retry...");
          vapiRef.current.stop();
        } catch (e) {
          console.error("Error stopping previous Vapi instance during retry:", e);
        } finally {
          // Ensure the ref is cleared to allow for a new instance
          vapiRef.current = null; 
          console.log("Vapi instance reference cleared for retry.");
        }
      }
      
      // Reinitialize and try again
      console.log("Re-initializing Vapi for retry...");
      ...
      if (vapiRef.current && callStatus !== "error") {
        console.log("Vapi re-initialized, attempting to start call again...");
        handleStartCall();
      } else {
        console.error("Vapi initialization failed during retry, aborting start call.");
  ...
  const resetState = () => {
    ...
    if (vapiRef.current) {
      try {
        if (isCallActive) { // If still marked active (e.g., unmount without dialog close event)
          console.log("resetState: Call was still marked active, stopping now.");
          vapiRef.current.stop();
        }
      } catch (error) {
        console.error("Error stopping Vapi during resetState:", error);
      } finally {
        vapiRef.current = null; 
      }
    }
          ...
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400 max-w-sm">
                Click the Start Call button below to begin talking with {agent.name}
              </p>
            </div>
          ) : (
            <>
