...
export function AgentTrainingPopup({
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[650px] md:max-w-[700px] p-0 flex flex-col h-[750px] max-h-[85vh] overflow-hidden bg-background">
          ...
          {messages.map(message => <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[50%] rounded-2xl px-4 py-2 shadow-sm relative group ${message.role === "user" ? "bg-blue-500 text-white rounded-br-none" : message.feedback === "negative" ? "bg-red-100 dark:bg-red-900 rounded-bl-none" : message.feedback === "positive" ? "bg-green-100 dark:bg-green-900 rounded-bl-none" : "bg-white dark:bg-gray-800/30 rounded-bl-none"}`}>
                {message.role === "agent" && <div className="absolute top-1/2 -translate-y-1/2 -right-28 flex space-x-2">
                    <Button variant="ghost" size="icon" className={`h-9 w-9 p-0 bg-white dark:bg-gray-700 rounded-full shadow-sm opacity-40 group-hover:opacity-100 transition-opacity ${speakingMessageId === message.id ? "text-blue-500 dark:text-blue-400 opacity-100" : "text-gray-500 dark:text-gray-400"}`} onClick={() => playTextToSpeech(message.id, message.content)} title="Listen to AI response">
                      <Volume2 className="h-5 w-5" />
                    </Button>
                    
                    <Button variant="ghost" size="icon" className={`h-9 w-9 p-0 bg-white dark:bg-gray-700 rounded-full shadow-sm opacity-40 group-hover:opacity-100 transition-opacity ${message.feedback === "negative" ? "text-red-500 dark:text-red-400 opacity-100" : "text-gray-500 dark:text-gray-400"}`} onClick={() => handleOpenCorrection(message.id)} title="Correct this response">
                      <AlertCircle className="h-5 w-5" />
                    </Button>
                  </div>}
                
                {editingMessageId === message.id ? <div className="mt-2">
                    <p className="text-xs mb-1 text-gray-600 dark:text-gray-300">
                      Provide the correct answer:
                    </p>
                    <div className="flex flex-col space-y-2">
                      <Textarea value={correctionInput} onChange={e => setCorrectionInput(e.target.value)} onKeyDown={handleKeyDown} className="min-h-[80px] text-sm border border-gray-300 dark:border-gray-700" placeholder="Enter the correct response..." />
                    ...
                    {message.correction && <div className="mt-2 p-2 bg-white dark:bg-gray-950 rounded border border-green-300 dark:border-green-700">
          ...
          {isTyping && <div className="flex justify-start">
              <div className="max-w-[50%] rounded-2xl px-4 py-2 bg-white dark:bg-gray-800/30 rounded-bl-none">
                <div className="flex space-x-1 h-5 items-center justify-center">
                  ...
                  <div className="w-2 h-2 rounded-full bg-gray-400 animate-pulse" style={{
        ...
        {showCorrectionSlider && <div className="absolute bottom-0 left-0 right-0 bg-white dark:bg-gray-950 border-t border-gray-200 dark:border-gray-800 p-4 transition-transform duration-300 transform translate-y-0 z-20">
            ...
            <Textarea value={correctionInput} onChange={e => setCorrectionInput(e.target.value)} onKeyDown={handleKeyDown} className="min-h-[100px] text-sm border border-gray-300 dark:border-gray-700 mb-3" placeholder="Enter the correct response the AI should have given..." autoFocus />
            <div className="flex justify-end space-x-2">
              <Button variant="ghost" size="sm" onClick={handleCancelCorrectionSlider}>
                Cancel
              </Button>
              <Button variant="default" size="sm" onClick={handleSubmitCorrectionFromSlider} disabled={!correctionInput.trim()}>
                Submit Correction
              </Button>
            </div>
          </div>}
        ...
        <div className={`p-4 border-t bg-white dark:bg-gray-950 ${showCorrectionSlider ? 'opacity-0 pointer-events-none' : 'opacity-100'} transition-opacity duration-200`}>
          {!editingMessageId && <div className="relative flex items-center">
              <Textarea placeholder="Type your message..." value={userInput} onChange={e => setUserInput(e.target.value)} onKeyDown={handleKeyDown} className="min-h-[40px] h-10 max-h-[100px] resize-none pr-3 rounded-md border-gray-300 dark:border-gray-700 focus-visible:outline-none focus-visible:ring-0 text-sm py-2 shadow-sm flex-1" />
              <Button onClick={handleSendMessage} disabled={!userInput.trim()} className="ml-2 h-10 py-2.5 rounded-md bg-blue-500 hover:bg-blue-600 transition-colors" size="sm">
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            <Button variant="outline" size="sm" className="text-xs h-8 rounded-md" onClick={() => onOpenChange(false)}>
