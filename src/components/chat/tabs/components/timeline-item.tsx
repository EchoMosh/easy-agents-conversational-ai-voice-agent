...
export function TimelineItemComponent({
  ...
  return <div className="relative">
      <div className="flex items-start gap-3">
        <div className="relative z-10 px-[8px]">
          ...
          <div className="p-3 rounded-lg bg-white/60 backdrop-blur-sm border border-white/40 shadow-sm">
            {isNote(item) && editingNoteId === item.id ? <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Button size="sm" onClick={() => onEditNote(item.id)} className="h-7 bg-white/80 text-black hover:bg-white/90">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {isNote(item) && <Button size="sm" variant="ghost" onClick={() => onEditNote(item.id)} className="h-7 px-2 hover:bg-white/60">
                      <Pencil className="h-3 w-3" />
                    </Button>}
                </div>
                {isNote(item) && <p className="text-sm text-gray-600 mt-1 line-clamp-3">
                    {item.content}
                  </p>}
                {!isNote(item) && <p className="text-xs text-gray-500 mt-1">
