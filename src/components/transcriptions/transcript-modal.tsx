...
interface TranscriptModalProps {
  ...
  onClose: () => void;
  vapiApiKey: string;
  agents: { id: string; name: string; assistantId: string; workspace_id: string }[];
}

const TranscriptModal: React.FC<TranscriptModalProps> = ({ callId, isOpen, onClose, vapiApiKey, agents }) => {
  ...
  useEffect(() => {
    if (callDetails) {
      setMessageRefs(callDetails.messages.map(() => React.createRef<HTMLDivElement>()));
    }
  }, [callDetails]);
      ...
      const fetchTranscriptDetails = async () => {
        ...
        try {
          ...
          if (!response.ok) {
            ...
            throw new Error(errorData.message || `Failed to fetch transcript details: ${response.statusText}`);
          }
          const data: VapiCallDetails = await response.json();
          let cumulativeOffset = 0;
          const messagesWithOffset = data.messages.map((msg, index, arr) => {
            let offsetForThisMessage = msg.timeOffset;
            if (offsetForThisMessage === undefined) {
                if (index > 0) {
                    const prevTimestamp = new Date(arr[index-1].timestamp).getTime();
  ...
  const renderMessageContent = (msg: Message): string => {
    if (msg.role === 'assistant' && typeof msg.message === 'object' && msg.message !== null) {
      // @ts-ignore
      if (msg.message.type === 'function-call') return `Function Call: ${msg.message.functionCall.name}`;
      // @ts-ignore
  ...
  const formatDateForDetails = (isoString: string | undefined) => {
    ...
    return date.toString() === 'Invalid Date' ? "N/A" : date.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });
  };
  ...
  const handleMessageClick = (message: Message) => {
    if (message.timeOffset !== undefined && audioPlayerRef.current) {
      ...
      audioPlayerRef.current.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
    }
  };

  const customerName = callDetails?.customer?.name || callDetails?.customer?.number || 'Caller';
  const agentName = getAgentNameById(callDetails?.assistantId);
  const agentInitial = agentName?.charAt(0).toUpperCase() || 'A';

  if (!isOpen) return null;

  const filteredMessages = callDetails?.messages.filter(msg => msg.role !== 'system') || [];

  const renderMessagesElements = (messagesToRender: Message[]) => {
    // Log each message's role to help debug
    console.log('Messages with roles:', messagesToRender.map(m => ({
      role: m.role,
    ...
    return messagesToRender.map((msg, i) => {
      ...
      const originalIndex = callDetails?.messages.findIndex(originalMsg => 
        originalMsg.timestamp === msg.timestamp && renderMessageContent(originalMsg) === originalContent
      ) ?? -1;
      ...
      return (
        ...
        >
          ...
          <div className={`max-w-[85%] px-3.5 py-2.5 rounded-2xl shadow-sm cursor-pointer ${isAgent ? 'bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-slate-100 rounded-bl-none' : 'bg-blue-500 text-white rounded-br-none'}`}>
  ...
  const DetailItem: React.FC<{ label: string; value: React.ReactNode }> = ({ label, value }) => (
    <div className="flex justify-between items-baseline">
      ...
      <dd className="text-sm text-slate-700 dark:text-slate-200 text-right">{value}</dd>
    </div>
  );
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(openState) => { if (!openState) onClose(); }}>
      ...
      <DialogContent className="max-w-4xl w-full sm:w-[calc(100%-3rem)] flex h-[calc(100vh-5rem)] max-h-[700px] bg-white dark:bg-gray-800 rounded-xl shadow-2xl overflow-hidden">
        {isLoading && <div className="flex items-center justify-center h-full text-lg p-10">Loading...</div>}
        {error && <div className="flex items-center justify-center h-full text-red-500 p-10 text-center">Error: {error}</div>}
        
        {callDetails && !isLoading && !error && (
          <>
            {/* Left Column */}
            <div className="w-[320px] flex-shrink-0 bg-slate-50 dark:bg-slate-800/30 border-r border-slate-200 dark:border-slate-700 flex flex-col pt-3"> {/* Added pt-3 to give space from top if DialogContent padding is not enough for X */}
              <div className="p-6 pb-4 pt-3"> {/* Adjusted padding */}
                <h2 className="text-2xl font-semibold text-slate-900 dark:text-slate-50 mb-1">Call Transcript</h2>
              ...
              <ScrollArea className="flex-1 px-6">
                <div className="space-y-6 pb-6">
                  <div>
                    <h3 className="text-md font-semibold mb-3 text-slate-800 dark:text-slate-100">Call Details</h3>
                    <dl className="space-y-2">
                      <DetailItem label="Agent" value={agentName} />
                      <DetailItem label="Date" value={formatDateForDetails(callDetails.startedAt || callDetails.createdAt)} />
                      <DetailItem label="Time" value={formatTimeInMinutes(callDetails.duration)} />
                      <DetailItem label="Type" value={callDetails.type ? callDetails.type.replace(/([A-Z])/g, ' $1').trim() : 'N/A'} />
                  ...
                  {callDetails.summary && (
                    <div>
                      ...
                      <h3 className="text-md font-semibold mb-2 text-slate-800 dark:text-slate-100">Summary</h3>
                      <div className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
                        <p className="whitespace-pre-wrap">{callDetails.summary}</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {/* Right Column */}
            <div className="flex-1 flex flex-col bg-white dark:bg-slate-800 overflow-hidden pt-3"> {/* Added pt-3 */}
              <div className="px-5 py-3 border-b border-slate-200 dark:border-slate-700"> {/* Adjusted padding */}
              ...
              <ScrollArea className="flex-1 px-5 pt-4 pb-5"> {/* Adjusted padding */}
              ...
              {callDetails.recordingUrl && (
                <div className="px-5 py-3 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
                  <div className="max-w-2xl mx-auto">
                    ...
                    <audio ref={audioPlayerRef} src={callDetails.recordingUrl} className="hidden" />
                  </div>
                </div>
              )}
            </div>
