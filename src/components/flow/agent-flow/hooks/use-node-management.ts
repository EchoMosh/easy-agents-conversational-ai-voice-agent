...
export function useNodeManagement() {
  ...
  const handleContextMenuAddNode = useCallback(
    (
      ...
      if (reactFlowWrapperRef.current) {
        ...
        toast.success(
          `Added ${widgets.find((w) => w.type === nodeType)?.label || nodeType} node`,
        );
      }
    },
    [
  ...
  const handleNodeContextMenu = useCallback(
    (
      ...
      setTimeout(() => {
        ...
        console.log("[Flow] Right-clicked node ID set to:", node.id);
      }, 0);
    },
    [],
  );
  ...
  const handlePaneContextMenu = useCallback(
    (
      ...
      console.log(
        "[Flow] Pane context menu triggered at:",
        event.clientX,
        event.clientY,
      );
      ...
      if (reactFlowWrapperRef.current) {
        ...
        console.log("[Flow] Setting context menu position:", position);
        setContextMenuPosition(position);

        // Force a small delay to ensure React state updates properly
        setTimeout(() => {
          // Always set rightClickedNodeId to null to indicate pane click (not node click)
          // This will show the add node options in the context menu
          console.log(
            "[Flow] Setting rightClickedNodeId for pane context menu",
          );
          setRightClickedNodeId(null);
        }, 0);
      }
    },
    [setRightClickedNodeId, setContextMenuPosition],
  );
      ...
      if (nodeIdToDelete) {
        ...
        if (nodeToDelete) {
          ...
          Promise.all([
            ...
            console.log(
              "[Flow] Notifying parent about deleted node and related edges",
            );
