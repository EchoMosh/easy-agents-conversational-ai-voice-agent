    ...
    <Panel position="bottom-left" className="space-y-2">
      <div className="relative">
        <button
          ...
          className="p-2 rounded-full bg-primary text-primary-foreground shadow-lg transform transition-transform hover:scale-105  hover:bg-primary/90"
        >
          <Plus
            className={`h-5 w-5 transition-transform ${showWidgets ? "rotate-45" : ""}`}
          />
        </button>
        {showWidgets && (
          <div className="widget-panel absolute bottom-14 left-0 bg-background/95  supports-[backdrop-filter]:bg-background/60 p-4 rounded-2xl shadow-[0_4px_20px_-4px_rgba(0,0,0,0.1)] space-y-3 min-w-[180px] border border-white/20">
            <TooltipProvider>
              {availableWidgets.map(
                (
                  widget, // Use availableWidgets prop
                ) => (
                  <Tooltip key={widget.type}>
                    <TooltipTrigger asChild>
                      ...
                      >
                        ...
                        <kbd className="ml-auto px-1.5 py-0.5 bg-muted/50 rounded text-[10px] font-semibold text-muted-foreground">
                          {widget.shortcut}
                        </kbd>
                      </div>
                    </TooltipTrigger>
                    <TooltipContent
                      side="right"
                      className="bg-white/90 dark:bg-gray-950/90  border-none shadow-[0_4px_20px_-4px_rgba(0,0,0,0.1)]"
                    >
                      {widget.description}
                    </TooltipContent>
                  </Tooltip>
