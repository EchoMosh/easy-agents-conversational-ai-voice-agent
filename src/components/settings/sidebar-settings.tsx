...
export function SidebarSettings() {
    ...
    const loadSavedSettings = () => {
      ...
      if (savedItems) {
        try {
          ...
          console.error(
            "Failed to parse sidebar-settings from localStorage:",
            err,
          );
        }
      }
  ...
  const saveChanges = () => {
    ...
    window.dispatchEvent(
      new CustomEvent("sidebar-settings-changed", {
        ...
        prev.map((item) =>
          item.id === parentId
            ? {
                ...item,
                subItems: item.subItems?.map((subItem) =>
                  subItem.id === id ? { ...subItem, icon } : subItem,
                ),
              }
            : item,
        ),
        ...
        <div className="space-y-4">
          <div className="mb-4">
            <p className="text-sm text-muted-foreground mb-2">
              Drag items to reorder them, toggle visibility with the checkbox,
              or change icons
            </p>
          </div>
