...
export function useLeads(initialPipelineId?: string) {
  ...
  // Fetch tags
  const { data: rawAvailableTags = [], isLoading: isTagsLoading } = useQuery({
  ...
  const {
    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (searchQuery) {
          query = query.or(
            `name.ilike.%${searchQuery}%,email.ilike.%${searchQuery}%,phone.ilike.%${searchQuery}%,status.ilike.%${searchQuery}%`,
          );
        }
        ...
        if (error) {
          // PGRST103 = offset out of range — just return empty, not an error
          if (error.code === "PGRST103") {
            console.log("📄 No more leads at this offset, returning empty");
        ...
        console.log(
          `✅ Fetched ${data?.length || 0} leads (total: ${count || 0})`,
        );
  ...
  const [stableLoading, setStableLoading] = useState(
    isLeadsLoading || isPipelinesLoading || isTagsLoading,
  );
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (
        scrollPosition + threshold >= documentHeight &&
        !isLeadsFetching &&
        hasMoreLeads
      ) {
