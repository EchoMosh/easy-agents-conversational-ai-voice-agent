...
import { parseCSV, parseCSVRow } from "@/utils/csv-parser";
 ...
 */
export async function importLeads(leads: LeadData[], options: ImportOptions) {
  ...
  try {
    ...
    if (invalidLeads.length > 0) {
      throw new Error(
        `${invalidLeads.length} leads are missing required fields (first name and phone number)`,
      );
    }
    ...
    if (removeDuplicates) {
      ...
      if (phones.length > 0) {
        ...
        if (checkError) {
          throw new Error(
            `Error checking for existing leads: ${checkError.message}`,
          );
        }
    ...
    return {
      ...
      leadIds: (insertedLeads || []).map((l: { id: string }) => l.id),
    };
  } catch (error) {
...
export async function processAndImportLeads(
  ...
  try {
    // Parse CSV using the same shared parser as the column-mapper preview
    const parsed = parseCSV(csvData, hasHeaders);

    // Process each row into a lead object
    const leads: LeadData[] = [];

    for (const values of parsed.dataRows) {
