...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const { phoneNumberId, inboundAgentId, outboundAgentId } = await req.json() as UpdatePhoneNumberAgentsRequest;
    ...
    if (membershipError || !membership || membership.role !== 'admin') {
      return new Response(
        JSON.stringify({ error: 'Unauthorized: Only workspace admins can update phone numbers' }),
        { 
          status: 403,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' }
        }
    ...
    // If inboundAgentId is provided, verify it exists and belongs to the same workspace
    if (inboundAgentId) {
      ...
      // Update VAPI phone number if needed
        ...
        if (vapiApiKey) {
          try {
            const vapiResponse = await fetch(`https://api.vapi.ai/phone-number/${phoneNumber.vapi_phone_number_id}`, {
              ...
              body: JSON.stringify({
                assistantId: inboundAgent.v_agent_id
              }),
            });

            if (!vapiResponse.ok) {
              console.error('Failed to update VAPI phone number:', await vapiResponse.text());
            }
          } catch (vapiError) {
            console.error('VAPI update error:', vapiError);
            // Continue even if VAPI update fails
          }
        }
      }
    } else if (inboundAgentId === null && phoneNumber.vapi_phone_number_id) {
      // Remove assistant assignment in VAPI
      const vapiApiKey = Deno.env.get('VAPI_API_KEY');
    ...
    // If outboundAgentId is provided, verify it exists and belongs to the same workspace
    if (outboundAgentId) {
